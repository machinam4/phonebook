    ...
    <div class="card md:h-screen">
        <div class="card-body">
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                <form class="tablelst-form" action="{{ route('users.store') }}" method="POST">
                    @csrf
                    <div class="mb-3" id="modal-id">
                        ...
                        <input type="text" id="id-field" name="name"
                            ...
                            placeholder="Full Names" value="{{ old('name') }}" required>
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <select
                            ...
                            @foreach ($channels as $channel)
                                <option value="{{ $channel->id }}" @selected(old('role'))>{{ $channel->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('role')
