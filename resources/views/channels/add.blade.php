    ...
    <div class="card md:h-screen">
        <div class="card-body">
            <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
                <h5 class="text-16" id="exampleModalLabel">Add Channel</h5>
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto p-4">
                <form class="tablelst-form" action="{{ route('channels.store') }}" method="POST">
                    @csrf
                    <div class="mb-3" id="modal-id">
                        <label for="id-field" class="inline-block mb-2 text-base font-medium">Full Names</label>
                        <span class="text-red-500">*</span></label>
                        <input type="text" id="id-field" name="name" value="{{ old('name') }}"
                    ...
                    <div class="mb-3">
                        ...
                        <select
                            ...
                            @foreach ($shortcodes as $shortcode)
                                <option value="{{ $shortcode->shortcode }}" @selected(old('shortcode'))>
                                    {{ $shortcode->name }} -
                                    {{ $shortcode->shortcode }}
                                </option>
                            @endforeach
